...
export function DocumentPicker({
  ...
  const ListComponent = variant === 'compact' ? List : 'div';
  ...
  return (
    <div className="space-y-3">
      ...
      {docs.length === 0 && emptyCtaHref && emptyCtaLabel && (
      ...
      {docs.length > 0 && (
        <ListComponent
          className={cn(variant === 'compact' && 'space-y-0', variant === 'default' && 'space-y-2')}
        >
          {docs.map((d) => (
            <button
              key={d.id}
              ...
              className={cn(
                ...
                selectedId === d.id && variant === 'default' ? 'border-primary/10' : 'border-border/60',
              )}
            >
              <ListItemContent>
